...
Motoro KV derinimas su prop dydžiu yra pats svarbiausias veiksnys build'o efektyvumui ir motoro ilgaamžiškumui. Tikslas: laikyti tip speed efektyviame diapazone ir laikyti motoro temperatūrą protingą.
...
**Praktinis efektyvus diapazonas: 100–150 m/s tip speed esant hover/cruise throttle.**
...
Pavyzdys — 5" prop (0,127 m skersmuo), esant 20 000 RPM:
```
...
| Prop skersmuo | Maks. efektyvus RPM (150 m/s) | Tipinis KV su 4S (14,8 V) |
|---------------|----------------------------|--------------------------|
| 3" (76 mm)    | ~37 600 RPM                | ~3500–4500 KV          |
| 4" (102 mm)   | ~28 100 RPM                | ~2600–3200 KV          |
| 5" (127 mm)   | ~22 500 RPM                | ~2000–2500 KV          |
| 5" (127 mm)   | ~22 500 RPM                | ~1500–1800 KV su 6S   |
| 7" (178 mm)   | ~16 100 RPM                | ~1300–1600 KV su 4S–6S |
| 10" (254 mm)  | ~11 300 RPM                | ~700–900 KV su 6S       |

---

## Motoro apkrovos indeksas (thrust-to-weight)

Naudingas sanity-check kabant: kiekvienas motoras neša ketvirtadalį viso svorio.
...
TWR 4:1 yra tipiškas freestyle'ui, 3:1 tinka cinematic, o racing nori 6:1+. 4:1 kvadras pakelia savo paties svorį naudodamas tik ketvirtadalį prieinamo thrust — likusi dalis yra atsarga punch-out'ams.

---

## Prop pitch ir motoro parinkimas

Pitch — tai teorinis atstumas, kurį prop pasistumia per apsisukimą. Aukštesnis pitch = agresyvesnis įsikabinimas = daugiau greičio, bet daugiau drag = motoras dirba sunkiau.
...
4. **Apskaičiuok reikalingą KV:**
   ...
   ```
5. **Patikrink tip speed** esant įvertintam maks. RPM:
   ```
   Max RPM = KV × Max Voltage
   Tip Speed = (π × Diameter_m × MaxRPM) / 60
   ```
   Turėtų likti žemiau ~170 m/s; idealiai žemiau 150 m/s.
6. **Patikrink statoriaus dydį** — didesnis statorius (pvz. 2306 vs 2204) tvarko daugiau karščio esant tam pačiam KV, tad sunkesnės prop kombinacijos reikalauja didesnio statoriaus.
...
```
→ Tip speed esant tam apkrautam maksimumui: `π × 0,127 × 26 000 / 60 ≈ 173 m/s`. Tai virš 150 m/s efektyvumo ribos — normalu freestyle, kuris iškeičia dalį efektyvumo į punch (racing suka dar aukščiau).

Praktikoje **2000–2450 KV** motorai yra nusistovėjęs sweet spot 5" su 4S, atitinkantys aukščiau esančią greitą nuorodų lentelę. (Žymėjimas kaip „2306“ yra *statoriaus dydis* — 23 mm × 6 mm — ne KV vertė.)

---

## Prop parinkimo nykščio taisyklės

- **Skersmuo** — nulemtas rėmo šakos ilgio ir motoro tvirtinimo tarpo. Neviršyk rėmo ribų.
...
- **Tip speed** — visada patikrink apskaičiuotą tip speed prie max throttle. Jei jis viršija 180 m/s, atsisakai dalies efektyvumo ir generuoji nereikalingą triukšmą.
...
- Motorų thrust lentelės iš gamintojų matuojamos esant konkrečioms įtampoms su konkrečiais prop'ais — visada persitikrink savo tiksliai kombinacijai.
- Maži skirtumai tarp prop gamintojų (HQ, Gemfan, DAL) turi įtakos realiam RPM, thrust ir efektyvumui net ant nominaliai identiškų prop'ų.
